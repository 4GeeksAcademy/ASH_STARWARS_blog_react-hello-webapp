		...
		<div className="accordion-item">
			...
			<div className="d-flex overflow-auto">
							{store.people && store.people.length >0 && store.people.map((item, index) => (
									...
									<CharacterCard id={item.url.split("/")[5]} type={item.url.split("/")[4]} index={index} name={item.name} gender={item.gender} hair={item.hair_color} eyes={item.eye_color}/>
									</div>
								)
							)}
						</div>
		...
		<div className="accordion-item">
			...
			<div className="d-flex overflow-auto">
							{store.planets && store.planets.length >0 && store.planets.map((item, index) => (
									<div className="col-auto" key={index}>
									<PlanetsCard id={item.url.split("/")[5]} type={item.url.split("/")[4]} index={index} name={item.name} population={item.population} terrain={item.terrain}/>
									</div>
								)
							)}
						</div>
		...
		<div className="accordion-item">
			...
			<div className="d-flex overflow-auto">
							{store.vehicles && store.vehicles.length >0 && store.vehicles.map((item, index) => (
									<div className="col-auto" key={index}>
									<VehiclesCard id={item.url.split("/")[5]} type={item.url.split("/")[4]} index={index} name={item.name} model={item.model} crew={item.crew}/>
									</div>
								)
							)}
			</div>
